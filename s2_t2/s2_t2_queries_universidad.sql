...
SELECT d.nombre AS 'Nom departament', 
COUNT(pr.id_profesor) AS 'Número de professors' 
FROM departamento d JOIN profesor pr ON d.id = pr.id_departamento 
GROUP BY d.nombre ORDER BY COUNT(pr.id_profesor) DESC; 
...
SELECT g.nombre AS 'Nom grau', COUNT(a.nombre) AS "Número d'assignatures" 
...
SELECT g.nombre AS 'Nom del grau', a.tipo AS "Tipus d'assignatura", SUM(a.creditos) AS 'Total de crèdits' 
FROM grado g JOIN asignatura a ON g.id = a.id_grado 
GROUP BY a.tipo, g.nombre; 

-- 8. Retorna un llistat que mostri quants alumnes s'han matriculat d'alguna assignatura en cadascun dels cursos escolars. El resultat haurà de mostrar dues columnes, una columna amb l'any d'inici del curs escolar i una altra amb el nombre d'alumnes matriculats: 
SELECT c.anyo_inicio AS "Any d'inici curs", COUNT(al.id_alumno) AS "Número d'alumnes matriculats" 
FROM curso_escolar c 
LEFT JOIN alumno_se_matricula_asignatura al ON al.id_curso_escolar = c.id 
LEFT JOIN asignatura a ON a.id = al.id_asignatura 
